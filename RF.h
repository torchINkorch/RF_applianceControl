...
bool updateH = true;     // Flag for update to home
...
public:
    ...
    void getRFApplianceState() {
     ...
     /**
     * @brief Update the recieved state of the assigned Appliance via RF device.
     *        Updates the state of the Appliance based on received command.
     */
    void updatememApplianceState() {
        if (RFstate) assignedAppliance.toggleState();
        updateN = true;
        updateH = true;
    }
...
//     Serial.print(RFsilenceCount);
//     Serial.print(RFsilentAddr);
...
Appliance door1 = Appliance::createNewAppliance(3, OFF);  // Type 2 for light
Appliance door2 = Appliance::createNewAppliance(3, OFF);  // Type 2 for light
...
RF rf7 = RF::createNewRF(door1, OFF);
RF rf8 = RF::createNewRF(door2, OFF);
